...
const fullPlayerViewVariants = tv({
  slots: {
    ...
    description: 'text-[12px] font-normal text-white/60',
    tabs: 'w-full gap-0',
    tabsList: 'bg-white/15 backdrop-blur-sm rounded-full p-1 flex',
    tabsTrigger:
      'flex-1 rounded-full px-3 py-1 text-center font-medium transition-all duration-300 ease-in-out relative z-10 data-[state=active]:bg-white data-[state=active]:text-[#121212] text-white/70',
    tabsContent: 'w-full transition-opacity duration-300 ease-in-out mt-5',
...
const FullPlayerView = ({
  ...
  koreaPlayers: PlayerInterface[];
  yonseiPlayers: PlayerInterface[];
}) => {
  const { root, container, containerLeft, title, description, tabs, tabsList, tabsTrigger, tabsContent, playerGrid } =
    fullPlayerViewVariants();
      ...
      <Tabs defaultValue="korea" className={tabs()}>
        <div className={container()}>
          <div className={containerLeft()}>
            <div className={title()}>전체선수 보기</div>
            <div className={description()}>양교의 모든 선수들을 확인해보세요</div>
          </div>
          <TabsList className={tabsList()}>
            <TabsTrigger className={tabsTrigger()} value="korea">
              고려대
            </TabsTrigger>
            <TabsTrigger className={tabsTrigger()} value="yonsei">
              연세대
            </TabsTrigger>
          </TabsList>
        </div>
